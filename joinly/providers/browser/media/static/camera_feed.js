(() => {
	const { w: W, h: H, nBands } = window.__camConfig;
	const LOGO_SRC =
		"data:image/svg+xml," +
	...
	function fxSpeaking(ctx, cx, y, bands, alpha) {
		...
		for (let i = 0; i < N; i++) {
			...
			ctx.beginPath();
			ctx.roundRect(ox + i * gap - barW / 2, y - h, barW, h * 2, barW / 2);
			ctx.fill();
		}
	}
	...
	function fxShare(ctx, cx, cy, logoW, logoH, t, alpha) {
		...
		for (let i = 0; i < 3; i++) {
			...
			ctx.beginPath();
			ctx.roundRect(cx - w / 2, cy - h / 2, w, h, 6 + ease * 4);
			ctx.stroke();
		}
	}
	...
	function fxThinking(ctx, cx, cy, logoW, logoH, t, alpha) {
		...
		for (let i = 0; i < 3; i++) {
			...
			ctx.beginPath();
			ctx.arc(cx, cy, r + H * (0.002 + i * 0.006), base, base + len);
			ctx.stroke();
		}
		...
		for (let i = 0; i < 2; i++) {
			...
			ctx.fillStyle = "#ffffff";
			ctx.beginPath();
			ctx.arc(cx + dx, cy + dy, H * (0.005 + dotPulse * 0.002), 0, Math.PI * 2);
			ctx.fill();
		}
	}
	...
	function fxBusy(ctx, cx, y, t, alpha) {
		...
		const ease = norm < 0.5 ? 2 * norm * norm : 1 - 2 * (1 - norm) * (1 - norm);
		const x = dir > 0 ? cx - w + ease * w * 2 : cx + w - ease * w * 2;

		// Glow line
		const grad = ctx.createLinearGradient(x - H * 0.015, y, x + H * 0.015, y);
		...
		for (let i = 1; i <= 5; i++) {
			...
			const te = tn < 0.5 ? 2 * tn * tn : 1 - 2 * (1 - tn) * (1 - tn);
			const tx = dir > 0 ? cx - w + te * w * 2 : cx + w - te * w * 2;
			const fade = 1 - i / 6;
	...
	function _initCanvas() {
		...
		const smoothBands = new Float32Array(nBands);
		let t = 0;
		let status = "";
		let statusAlpha = 0;
		let statusT = 0;
		...
		window.__setBands = (b) => {
			for (let i = 0; i < bands.length; i++) bands[i] = b[i] || 0;
		};
		...
		function draw() {
			...
			if (logoImg) {
				...
				if (statusAlpha > 0.02) {
					...
					if (bgFn) {
						ctx.save();
						bgFn(ctx, cx, cy, logoW, logoH, statusT, statusAlpha);
						ctx.restore();
					}
				}
				...
				if (anyBand) {
					ctx.save();
					fxSpeaking(ctx, cx, logoBot + H * 0.04, smoothBands, 1);
					ctx.restore();
				}
				...
				if (statusAlpha > 0.02) {
					...
					if (fn) {
						ctx.save();
						fn(ctx, cx, logoBot + H * 0.08, statusT, statusAlpha);
						ctx.restore();
					}
				}
			}
	...
	RTCPeerConnection.prototype.addTrack = function (track, ...streams) {
		if (track.kind === "video") {
			return origAddTrack.call(this, _initCanvas().clone(), ...streams);
		}
